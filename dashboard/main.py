...
async def paper_status():
    """Get paper trading status and summary metrics."""
    import sqlite3
    ...
    if not jdb:
        return {"paper_trading": False, "message": "No journal.db found"}
    ...
    try:
        ...
        if "paper_trades" not in table_names:
            conn.close()
            return {"paper_trading": True, "trades_count": 0, "message": "No paper trades yet"}

        # Summary stats
        total = conn.execute("SELECT COUNT(*) as c FROM paper_trades WHERE event='EXIT'").fetchone()["c"]
        wins = conn.execute("SELECT COUNT(*) as c FROM paper_trades WHERE event='EXIT' AND net_pl > 0").fetchone()["c"]
        gross = conn.execute("SELECT COALESCE(SUM(gross_pl),0) as s FROM paper_trades WHERE event='EXIT'").fetchone()["s"]
        net = conn.execute("SELECT COALESCE(SUM(net_pl),0) as s FROM paper_trades WHERE event='EXIT'").fetchone()["s"]
        comm = conn.execute("SELECT COALESCE(SUM(commission),0) as s FROM paper_trades WHERE event='EXIT'").fetchone()["s"]
        ...
        return {"paper_trading": True, "error": str(e)}
...
async def paper_trades(limit: int = 100):
    ...
    jdb = None
    for p in [_repo("journal.db"), os.path.join(os.path.dirname(os.path.dirname(__file__)), "journal.db")]:
        if os.path.exists(p):
            jdb = p
            break
    if not jdb:
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        rows = conn.execute(
            "SELECT * FROM paper_trades WHERE event IN ('ENTRY','EXIT') ORDER BY timestamp DESC LIMIT ?",
            (limit,)
        ).fetchall()
        conn.close()
...
async def paper_equity():
    """Get equity curve data from paper trades."""
    jdb = None
    for p in [_repo("journal.db"), os.path.join(os.path.dirname(os.path.dirname(__file__)), "journal.db")]:
        if os.path.exists(p):
            jdb = p
            break
    if not jdb:
        return {"points": [], "current": 10000}
    try:
        conn = sqlite3.connect(jdb)
        conn.row_factory = sqlite3.Row
        rows = conn.execute(
            "SELECT timestamp, net_pl FROM paper_trades WHERE event='EXIT' ORDER BY timestamp ASC"
        ).fetchall()
        conn.close()
        equity = 10000.0
        points = []
        for r in rows:
            equity += r["net_pl"]
            points.append({"time": r["timestamp"], "equity": round(equity, 2)})
        return {"points": points, "current": round(equity, 2)}
    except:
        return {"points": [], "current": 10000}


@app.get("/api/paper/log")
async def paper_log(limit: int = 50):
    """Get paper trade log with combined entry/exit pairs."""
    import sqlite3
    jdb = None
    for p in [_repo("journal.db"), os.path.join(os.path.dirname(os.path.dirname(__file__)), "journal.db")]:
        if os.path.exists(p):
...
async def bot_health():
    """Get live bot health from heartbeat."""
    health_path = _repo("health.json")
    if not os.path.exists(health_path):
    ...
    try:
        ...
        if age < 120:
            ...
            status = "STOPPED"
        return {"status": status, "health": data, "age_seconds": age}
    except:
