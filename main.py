...
class BTCBotV13:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            await asyncio.shield(self._telegram.send("🔴 <b>BTC Bot v13 Bot Stopped</b>"))
        except Exception:
            pass
        try:
            self._journal.close()
        ...
        async with self._entry_lock:
            ...
            try:
                order = await self._order_mgr.place_entry(
                    ...
                    tp          = risk_pre.tp,
                    atr         = snap.atr,
                    entry_price = snap.close,  # paper mode fills here; live ignores it
                )
            except Exception as e:
...
from orders.manager     import create_order_manager, build_exchange          # noqa: E402,F401
