...
class PaperOrderManager:
    """
    Paper trading order manager — simulates orders without placing real trades.
    Logs everything to the journal database with is_paper=1 flag.

    Use this when PAPER_TRADING=true in .env.  The bot will:
      - Connect to Delta API READ-ONLY (price feed only)
      - Simulate entries at market price
      - Simulate exits at SL/TP prices
      - Log all paper trades to journal DB
      - NEVER place actual orders on the exchange
    """
    ...
    async def initialize(self) -> None:
        self.exchange = build_exchange()
        await self.exchange.load_markets()
        logger.info("PaperOrderManager initialized (NO real trading)")

    async def place_entry(self, is_long: bool, simulated_price: float = 0.0) -> dict:
        if simulated_price > 0:
            ...
            try:
                ...
                if fill_price <= 0: fill_price = 60000.0
            except Exception:
                fill_price = 60000.0
        side = "buy" if is_long else "sell"
        order = {"id": f"paper_{int(time.time()*1000)}", "symbol": SYMBOL,
                 "type": "market", "side": side, "amount": ALERT_QTY,
                 "price": fill_price, "average": fill_price, "status": "closed"}
        self.position = {"entry_order_id": order["id"], "is_long": is_long,
                         "entry_price": fill_price, "is_paper": True}
        logger.info(f"PAPER ENTRY {side.upper()} at {fill_price:.2f}")
        return order

    async def close_position(self, reason: str = "Exit", simulated_price: float = 0.0) -> dict:
        if not self.position:
            return {}
        is_long = self.position["is_long"]
        side = "sell" if is_long else "buy"
        ...
        qty = ALERT_QTY
        gross_pl = (fill_price - entry_price) * qty if is_long else (entry_price - fill_price) * qty
        comm_rate = 0.0005
        commission = entry_price * qty * comm_rate
        order = {"id": f"paper_exit_{int(time.time()*1000)}", "symbol": SYMBOL,
                 "type": "market", "side": side, "amount": ALERT_QTY,
                 "price": fill_price, "average": fill_price, "status": "closed",
                 "gross_pl": gross_pl, "commission": commission,
                 "net_pl": gross_pl - commission, "exit_reason": reason, "is_paper": True}
        logger.info(f"PAPER EXIT {side.upper()} at {fill_price:.2f} | "
                     f"P&L: ${gross_pl:.2f} net | {reason}")
        self.position = None
        return order

    async def fetch_position(self) -> Optional[dict]:
        if self.position:
            return {"symbol": SYMBOL, "contracts": ALERT_QTY,
                    "entry_price": self.position.get("entry_price", 0),
                    "side": "long" if self.position.get("is_long") else "short"}
        return None
...
def create_order_manager() -> OrderManager:
    """Factory: returns PaperOrderManager if PAPER_TRADING=true, else OrderManager."""
    if PAPER_TRADING:
        logger.info("=" * 50)
